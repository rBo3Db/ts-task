...
const normalizeData = (unnormalizedData: UnnormalizedData): NormalizedData => {
  // Your code here...
  const initialObject: NormalizedData = {
    byId: {},
    allIds: [],
  };
  const data = unnormalizedData.reduce((accumulator, currentValue) => {
    accumulator.allIds = [...accumulator.allIds, currentValue.id];
    accumulator.byId[currentValue.id] = {
      id: currentValue.id,
